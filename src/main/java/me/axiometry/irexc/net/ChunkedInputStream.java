 ...
 * 
 * <p>
 * Note that this class NEVER closes the underlying stream, even when close gets called. Instead, it will read until the "end" of its chunking on close, which
 * allows for the seamless invocation of subsequent HTTP 1.1 calls, while not requiring the client to remember to read the entire contents of the response.
 * </p>
 * 
...
public class ChunkedInputStream extends InputStream {
	...
	/**
	 * ChunkedInputStream constructor that associates the chunked input stream with a {@link HttpMethod HTTP method}. Usually it should be the same
	 * {@link HttpMethod HTTP method} the chunked input stream originates from. If chunked input stream contains any footers (trailing headers), they will be
	 * added to the associated {@link HttpMethod HTTP method}.
	 * 
	 * @param in
	...
	/**
	 * Upon close, this reads the remainder of the chunked message, leaving the underlying socket at a position to start reading the next response without
	 * scanning.
	 * 
	 * @throws IOException
	 *             If an IO problem occurs.
	 ...
	 * 
	 * <p>
	 * Note that this function is intended as a non-public utility. This is a little weird, but it seemed silly to make a utility class for this one function,
	 * so instead it is just static and shared that way.
	 * </p>
	 * 
	 * @param inStream
