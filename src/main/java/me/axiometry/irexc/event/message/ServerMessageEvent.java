...
import me.axiometry.irexc.Server;
...
public class ServerMessageEvent extends MessageEvent {
	...
	public ServerMessageEvent(String raw, Server server, int code, String target, String[] options, String message) {
		super(raw);
	...
	public String toString() {
		StringBuilder builder = new StringBuilder(getName()).append(": ").append(server.getName()).append(' ').append(code);
		if(target != null)
			builder.append(" to ").append(target);
		for(int i = 0; i < options.length; i++) {
			if(i == 0)
				builder.append(" (");
			builder.append(options[i]).append(i == options.length - 1 ? ")" : ", ");
		}
